...
class SequenceFeature(object):
    ...
    In recommendation, there are many user behavior features that we want to take the sequence model
    and tag features (multi hot) which we want pooling. Note that if you use this feature, you must be padding
    ...
    Args:
        ...
        shared_with (str): another feature name which this feature will share with embedding.
        padding_idx (int, optional): If specified, the entries at padding_idx will be masked 0 in the InputMask Layer.
